...
source(here::here("pipeline/utils/util.R"))
...
write.csv(tab_out, file=here::here("outputs/intermediate/tlg_var_filter_table.csv"), row.names = FALSE)
```
...
tab_var_label <- all_var |> left_join(all_name_label, by = c("Variable" = "name")) |> distinct()
```

```{r}
knitr::kable(tab_var_label)

write.csv(tab_var_label, file=here::here("outputs/intermediate/adam_var_label_table.csv"), row.names = FALSE)
```
...
```{r}
description_btw <- sapply(pkgdesc[,1], function(i)
  llm_call(btw(i,"write an one sentence description on this package's functionality."), 
           model = MODEL_NAME))

pkgdesc2 <- pkgdesc
pkgdesc2$Description <- description_btw

knitr::kable(pkgdesc2|> select(c("Package","Version","Description")))
```
